  ...
  const handleConnectionAction = async (connectionId: string, status: "approved" | "rejected") => {
    ...
    if (!error) {
      toast({
        ...
        className: status === "approved" ? "bg-green-600 text-white" : "bg-gray-800 text-white",
      });
      fetchConnections();
    } else {
          ...
          {myProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {myProducts.map((product) => (
                <Card
                  key={product.id}
                  className="rounded-2xl border-none shadow-lg hover:shadow-xl transition-all duration-500 bg-white overflow-hidden group"
                >
          ...
          <div className="grid gap-8 md:grid-cols-12">
            <div className="md:col-span-7 space-y-6">
              <Card className="rounded-3xl border-none shadow-xl bg-white overflow-hidden">
                <CardHeader className="bg-secondary/20 pb-6 pt-6">
                  <CardTitle className="text-2xl font-serif text-primary">
                    {editingId ? "Editando Peça" : "Ficha Técnica"}
                  </CardTitle>
                  {editingId && (
                    <Button
                      variant="ghost"
                ...
                <CardContent className="p-8 space-y-6">
                  <div className="grid gap-2">
                    ...
                    <Input
                      className="h-12 rounded-xl bg-secondary/10 border-transparent focus:bg-white focus:border-primary/20 transition-all"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      ...
                      <Input
                        className="h-12 rounded-xl bg-secondary/10 border-transparent focus:bg-white focus:border-primary/20"
                      ...
                      <div className="relative h-48 rounded-2xl overflow-hidden group">
                        ...
                        >
                          <span className="text-white font-medium flex items-center gap-2">
                            <UploadCloud className="h-5 w-5" /> Alterar
                          </span>
                        </div>
                      </div>
                    ) : (
                      <div
                        className="h-48 border-2 border-dashed border-border rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-secondary/20 transition-colors"
                        onClick={() => document.getElementById("up-img")?.click()}
                      >
                  ...
                  <div className="grid gap-2">
                    <Label>Conceito / Descrição</Label>
                    <Textarea
                      className="min-h-[100px] rounded-xl bg-secondary/10 border-transparent focus:bg-white focus:border-primary/20 p-4"
              ...
              <Card className="rounded-3xl border-none shadow-lg bg-white">
                <CardHeader className="pb-2">
                  <CardTitle className="text-lg font-serif flex items-center gap-2">
                    <Layers className="h-5 w-5 text-primary" /> Composição
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-6 pt-2">
                  {selectedMaterials.length === 0 ? (
                    <p className="text-sm text-muted-foreground italic">Nenhum material selecionado.</p>
                  ) : (
                    Object.entries(groupedSelected).map(([grp, its]) => (
                        ...
                        <div className="flex flex-wrap gap-2">
                          {its.map((m) => (
                            <Badge
                              key={m.id}
                              variant="secondary"
                              className="cursor-pointer hover:bg-destructive hover:text-white transition-colors py-1 px-3 rounded-lg"
                              onClick={() => toggleMaterial(m)}
                            >
                              {m.name} <X className="w-3 h-3 ml-2 opacity-50" />
                            </Badge>
                          ))}
                        </div>
                      </div>
                ...
                <div className="p-6 bg-secondary/10 rounded-b-3xl">
                  <Button
                    className="w-full h-14 text-lg font-serif rounded-2xl shadow-xl"
                    onClick={handleSaveProduct}
                    disabled={loading}
                  >
                    {loading ? <Loader2 className="animate-spin" /> : "Publicar Ficha Técnica"}
                  </Button>
                </div>
              </Card>
            </div>
            <div className="md:col-span-5 space-y-6">
              <Card className="rounded-3xl border-none shadow-xl bg-white flex flex-col h-[1000px]">
                <CardHeader className="pb-6 px-8 pt-8 border-b border-border bg-secondary/10">
                  <CardTitle className="text-xl font-serif text-primary flex items-center gap-2">
                    <Building2 className="h-5 w-5" /> Curadoria de Materiais
                  </CardTitle>
                  <div className="space-y-3 mt-6">
                    ...
                    >
                      <SelectTrigger className="h-12 bg-white border-transparent shadow-sm rounded-xl">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <ScrollArea className="h-full p-6">
                    <div className="grid grid-cols-2 gap-4">
                      {displayedMaterials.map((m) => {
                        const sel = selectedMaterials.some((x) => x.id === m.id);
                        return (
                          <div
                            key={m.id}
                            onClick={() => toggleMaterial(m)}
                            className={`p-3 border rounded-2xl cursor-pointer transition-all duration-300 ${sel ? "border-primary ring-1 ring-primary bg-primary/5" : "border-border bg-white hover:border-primary/30 hover:shadow-md"}`}
                          >
                            <div className="h-24 bg-secondary/20 rounded-xl mb-3 overflow-hidden">
                              {m.image_url && <img src={m.image_url} className="w-full h-full object-cover" />}
                            </div>
                            <p className="text-sm font-serif font-medium truncate">{m.name}</p>
                            <p className="text-[10px] text-muted-foreground tracking-wide mt-1 uppercase">
                              {m.supplier_name}
                            </p>
                          </div>
                        );
